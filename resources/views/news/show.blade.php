...
<div class="bg-gray-50 py-16 px-4 sm:grid sm:grid-cols-12">
    <div class="sm:col-start-4 sm:col-span-6">
        ...
        <div class="bg-white mt-4 px-4 py-6 rounded-sm shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <div class="flex text-sm leading-5 text-gray-500">
                        <time datetime="{{ $new->created_at }}">
                            {{ $new->created_at->diffForHumans() }}
                        </time>
                        <span class="mx-1">&middot;</span>
                        <span>{{ ceil(strlen($new->text) / 863) }} min read</span>
                    </div>
                </div>
            </div>
            ...
            <h2 class="mt-6 text-4xl leading-10 tracking-tight font-bold text-gray-900 text-center">{{ $new->title }}</h2>
            <p class="mt-6 leading-6 text-gray-500">{{ $new->text }}</p>
        </div>

        <h2 class="mt-6 text-4xl leading-10 tracking-tight font-bold text-gray-900 text-center">Comments</h2>

        <div>
            <form action="/news/{{ $new->id }}/comments" method="POST" class="mb-0">
                @csrf
